...
thisEvent = possibleEvents[event.type]
   ...
   ; Do we have a valid window object? If not, initialize window.

;IF Obj_Valid(self.theWindow) EQ 0 THEN BEGIN
;   Widget_Control, event.id, Get_Value=theWindow
;   self.theWindow = theWindow
;   self->SetPageSize, self.pagesize, Landscape=self.landscape
;   self->SetWindowSize, [self.x1, self.y1, self.x2, self.y2]
;ENDIF
...
FUNCTION FSC_PLOTWINDOW::INIT, parent, PageSize=pagesize, WindowSize=windowsize, $
   Event_Pro=event_pro, Units=units, UValue=uvalue, Landscape=landscape, $
   Color=color, Debug=debug, WindowColor=windowColor
