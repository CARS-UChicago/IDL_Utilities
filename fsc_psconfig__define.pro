...
IF self.fontInfo THEN BEGIN
   ...
   IF self.fontTypeSet EQ -1 THEN BEGIN
      ...
      Widget_Control, self.set_fontID->GetID(), Sensitive=0
   ENDIF ELSE BEGIN
      ...
      Widget_Control, self.set_fontID->GetID(), Sensitive=1
...
IF self.fontInfo THEN BEGIN
   ...
   self.set_fontID->Set_Value, self.set_fontSet
...
; page window). Safe eqal 1 means the current configuration is saved.
; Save equal 0 means the current configuration is not saved. The
...
ENDELSE
   ...
   ; Update the background window color.

self.plotID->SetWindowColor, windowColor
END ;--------------------------------------------------------------------------------
...
IF self.fontInfo THEN BEGIN
   ...
   self.set_fontSet = self.Set_FontID->Get_Value()
...
IF buttonValue EQ 'Accept' AND Widget_Info(self.tlb, /Valid_ID) THEN $
...
shortside = windowDims[0]
longside = windowDims[1]
IF self.landscapeSet THEN BEGIN
...
self.xsizeID = FSC_InputField(sizebase, Title='X Size: ', Value=self.xsizeSet, $
   /Float, XSize=CW_FIELD_SIZE, Decimal=2, /CR_Only, Event_Pro='FSC_PSCONFIG_Events', $
   UValue={Method:'WINDOWSIZE', Object:self}, /Positive)

self.ysizeID = FSC_InputField(sizebase, Title=' Y Size: ', Value=self.ysizeSet, $
   /Float, XSize=CW_FIELD_SIZE, Decimal=2, /CR_Only, Event_Pro='FSC_PSCONFIG_Events', $
   UValue={Method:'WINDOWSIZE', Object:self}, /Positive)

self.xoffsetID = FSC_InputField(sizebase, Title=' X Offset: ', Value=self.xoffsetSet, $
   /Float, XSize=CW_FIELD_SIZE, Decimal=2, /CR_Only, Event_Pro='FSC_PSCONFIG_Events', $
   UValue={Method:'WINDOWSIZE', Object:self}, /Positive)

self.yoffsetID = FSC_InputField(sizebase, Title=' Y Offset: ', Value=self.yoffsetSet, $
   /Float, XSize=CW_FIELD_SIZE, Decimal=2, /CR_Only, Event_Pro='FSC_PSCONFIG_Events', $
   UValue={Method:'WINDOWSIZE', Object:self}, /Positive)
...
labelBase = Widget_Base(leftTopBase, Column=1, Base_Align_Center=1)
...
dummy = FSC_FileSelect(rightTopBase, Filename=self.filenameSet, $
   Directory=self.directorySet, XSIZE=35, ObjectRef=filenameID)
...
self.acceptID = Widget_Button(actionBase, Value=acceptButVal, UValue={Method:'ACCEPT', Object:self})
...
targetsize = t.scr_xsize
self.set_fontID->Resize, targetsize
...
PRO FSC_PSCONFIG::GUI, Group_Leader=leader, NoBlock=noblock, Cancel=cancel, FontInfo=fontinfo
...
dummy = FSC_FileSelect(rightTopBase, Filename=self.filenameSet, $
   Directory=self.directorySet, XSIZE=25, ObjectRef=filenameID)
self.filenameID = filenameID
